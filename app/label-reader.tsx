...
function parsePlateText(rawText: string): Form {
  ...
  (Object.keys(aliases) as FieldKey[]).forEach((key) => {
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const labelOnly = labels.some((label) => normalizedLine === label.toLowerCase());
      const nextLineValue = labelOnly ? clean(lines[index + 1] ?? "") : "";
      const value = inlineValue || nextLineValue;
      if (value && value.length > 0 && value.length < 80 && value.toLowerCase() !== line.toLowerCase()) {
        result[key] = value;
        break;
      }
    }
