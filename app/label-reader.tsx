...
const FIELDS = [
  ["Brand", "brand", "Brand", "ماركة الجهاز كما هي مكتوبة على اللوحة."],
...
function parsePlateText(rawText: string): Form {
  ...
  read("brand", ["Manufactured By", "Brand", "Manufacturer", "Made By"]);
  if (!result.brand) {
    const knownBrand = rawText.match(/\b(Carrier|Fujisti|Fujitsu|Gree|Midea|LG|Samsung|Daikin|Trane|York|Zamil|O General|Toshiba|Hisense)\b/i);
...
type SummaryItem = { label: string; value: string; source: string };
...
const buildSummary = (form: Form): SummaryItem[] => {
  const coolingText = form.coolingCapacity;
  const coolingNumber = numberFrom(coolingText);
  let btu = 0;
  if (/btu/i.test(coolingText)) btu = coolingNumber;
  else if (/kw/i.test(coolingText)) btu = coolingNumber * 3412.142;
  else if (/ton/i.test(coolingText)) btu = coolingNumber * 12000;
  const tons = btu > 0 ? btu / 12000 : 0;
  const hpClasses = [
    [9000, 1.25], [12000, 1.5], [18000, 2.25], [24000, 3], [36000, 4], [48000, 5], [60000, 6],
  ] as const;
  const nearest = hpClasses.reduce((best, item) => Math.abs(item[0] - btu) < Math.abs(best[0] - btu) ? item : best, hpClasses[0]);
  const summary: SummaryItem[] = [
    { label: "Brand", value: form.brand, source: "مقروءة من اللوحة" },
    { label: "Model", value: form.model, source: "مقروءة من اللوحة" },
    { label: "Cooling Capacity", value: form.coolingCapacity, source: "مقروءة من اللوحة" },
    { label: "Estimated TON", value: tons > 0 ? `${tons.toFixed(2)} TON` : "", source: "محسوبة من قدرة التبريد" },
    { label: "Estimated HP", value: btu > 0 ? `${nearest[1]} HP تقريباً` : "", source: "تقدير تجاري من الحمل" },
    { label: "Refrigerant / Charge", value: [form.refrigerant, form.charge].filter(Boolean).join(" / "), source: "مقروءة من اللوحة" },
    { label: "Current", value: form.ratedCoolingCurrent || form.ratedHeatingCurrent, source: "مقروءة من اللوحة" },
    { label: "Pressure", value: form.maxDischargePressure || form.maxSuctionPressure || form.designPressure, source: "مقروءة من اللوحة" },
  ];
  return summary;
};
